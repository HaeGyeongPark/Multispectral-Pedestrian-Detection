...
def save_annotations(gts, dts):
        global counter
        counter += 1

        # Convert the list of dictionaries to a JSON string
        gts_json = json.dumps(gts, indent=4)
        dts_json = json.dumps(dts, indent=4)

        # Create file names with the counter
        gts_file_name = f"./gts_annotations_{counter}.txt"
        dts_file_name = f"./dts_annotations_{counter}.txt"

        # Write the JSON strings to text files
        with open(gts_file_name, "w") as gts_file:
            gts_file.write(gts_json)
        
        with open(dts_file_name, "w") as dts_file:
            dts_file.write(dts_json)
